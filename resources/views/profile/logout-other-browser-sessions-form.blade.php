<x-jet-action-section>
    ...
    <x-slot name="content">
        ...
        <div>
            إذا لزم الأمر ، يمكنك تسجيل الخروج من جميع جلسات المتصفح الأخرى عبر جميع أجهزتك. بعض جلساتك الأخيرة مذكورة أدناه ؛ ومع ذلك ، قد لا تكون هذه القائمة شاملة. إذا شعرت أنه تم اختراق حسابك ، يجب عليك أيضًا تحديث كلمة المرور الخاصة بك.        </div>
                            ...
                            <div>
                                <div class="small font-weight-lighter text-muted">
                                    ...
                                    @if ($session->is_current_device)
                                        <span class="text-success font-weight-bold">هذا الجهاز</span>
                                    @else
                                        اخر نشاط {{ $session->last_active }}
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        <x-jet-dialog-modal wire:model="confirmingLogout">
            ...
            <x-slot name="content">
                ...
                <div class="mt-3 w-md-75" x-data="{}" x-on:confirming-logout-other-browser-sessions.window="setTimeout(() => $refs.password.focus(), 250)">
                    <x-jet-input type="password" placeholder="كلمة المرور"
                                 x-ref="password"
